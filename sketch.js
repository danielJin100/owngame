...
for(var v = 0; v < tesseract.length; v++){
  ...
  fill(0);
  if(level === 1){
      text('use Q and E to rotate', 400, 30);
  }
  if(level === 2){
      text("use WASD to rotate", 400, 30);
  }
  if(level === 3){
    text("use U, O, J, and L to rotate", 400, 30);
  }
...
function rot2(){
  comp_rot = [];
  var angle11 = compRotation[0];
  var angle12 = compRotation[1];
  var angle13 = compRotation[2];
  var angle14 = compRotation[3];
  var angle15 = compRotation[4];
  var angle16 = compRotation[5];
  for(var f = 0; f < 16; f++){
    comp_rot.push({
...
function keyPressed(){
  if(keyCode === ENTER){
    var list = [];
  for(var i = 0; i < 6; i++){
    list.push(Math.abs(compRotation[i]-rotation[i]));
    console.log(list[i]);
  }
  var diff = (Math.max(list));
  console.log(diff);
  if(diff < 20 || Number.isNaN(diff) && level > 1){
    level++;
    compRotation = [];
    rotation = [0,0,0,0,0,0];
    for(var i = 0; i < 5; i++){
      compRotation.push(Math.round(Math.random()*359));
    }
    } else if(diff < 20 || Number.isNaN(diff) && level == 1){
      compRotation = [45, 45, 10, 0, 0, 0]
      level++;
    } 
  }
